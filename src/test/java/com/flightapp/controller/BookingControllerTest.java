...
class BookingControllerTest {
    ...
    void bookTicket_success() throws Exception {
        ...
        Booking saved = Booking.builder().id(10L).pnr("A1B2C3").email(req.getEmail())
                .bookingTime(LocalDateTime.now())
                .journeyDateTime(LocalDateTime.of(2025, 11, 25, 14, 30))
                .departureTime(LocalDateTime.of(2025, 11, 25, 14, 30))
                .arrivalTime(LocalDateTime.of(2025, 11, 25, 16, 30)).totalPrice(9000.0).flight(flight)
                .passengers(List.of(
        ...
        mockMvc.perform(post("/api/v1.0/flight/booking/1")
                ...
                .andExpect(jsonPath("$.flight.fromPlace").value("Delhi"))
    ...
    void bookTicket_notEnoughSeats_returnsBadRequest() throws Exception {
        ...
        Mockito.when(bookingService.bookTicket(eq(1L), any(BookingRequestDto.class))).thenThrow(new RuntimeException("Not enough seats available"));
    ...
    void getTicketByPnr_success() throws Exception {
        ...
        Flight flight = Flight.builder()
                .flightNumber("6E-512").airlineName("IndiGo").fromPlace("Delhi").toPlace("Mumbai")
                .build();

        Booking saved = Booking.builder().id(10L)
                .pnr("A1B2C3").email("testuser@gmail.com")
                .journeyDateTime(LocalDateTime.of(2025,11,25,14,30)).totalPrice(9000.0).flight(flight)
                .passengers(List.of(
                        Passenger.builder().name("Rohit").build()
                ))
                .build();
    ...
    void getTicketByPnr_notFound() throws Exception {
        ...
        Mockito.when(bookingService.getByPnr("BADPNR")).thenThrow(new NotFoundException("PNR not found"));
    ...
    void history_success() throws Exception {
        ...
        Flight f1 = Flight.builder().flightNumber("6E-111").fromPlace("Delhi").toPlace("Mumbai").build();
        Flight f2 = Flight.builder().flightNumber("6E-222").fromPlace("Delhi").toPlace("Bengaluru").build();

        Booking b1 = Booking.builder().id(10L).pnr("P1").flight(f1)
                .bookingTime(LocalDateTime.now()).totalPrice(4500.0).build();

        Booking b2 = Booking.builder().id(11L).pnr("P2").flight(f2)
                .bookingTime(LocalDateTime.now()).totalPrice(6000.0).build();
    ...
    @Test
    void cancelBooking_pnrNotFound_returnsNotFound() throws Exception {

        Mockito.doThrow(new NotFoundException("PNR not found")).when(bookingService).cancelBooking("XYZ123");
    ...
    void cancelBooking_within24hrs_returnsBadRequest() throws Exception {
        ...
        Mockito.doThrow(new ExceptionDuetoTiming("Cannot cancel within 24 hours of journey")).when(bookingService).cancelBooking("A1B2C3");

        mockMvc.perform(delete("/api/v1.0/flight/booking/cancel/A1B2C3"))
                .andExpect(status().isBadRequest()).andExpect(jsonPath("$.error").value("Cannot cancel within 24 hours of journey"));
    }
}
